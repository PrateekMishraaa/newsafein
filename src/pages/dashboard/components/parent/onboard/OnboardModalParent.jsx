...
const onboardSchema = new Yup.object({
  ...
  coordinator: Yup.object()
    .shape({
      name: Yup.string().max(400).required("School Safety Coordinator's Name is required"),
      email: Yup.string().email("email must be a valid.").max(300).required("School Safety Coordinator's Email is required"),
      contact: Yup.string()
        .required("School Safety Coordinator's Contact Number is required")
        .max(300)
        .matches(/^[0-9]{10}$/, "Invalid Contact Number"),
      designation: Yup.string().max(300).required("Coordinator's Designation is required"),
    })
    .required("Must have Coordinator"),
});

// Onboarding Steps
const steps = ["Tell us about yourself", "More information about your school and role", "More about yourself"];
function OnboardStepper({ handleClose, details }) {
  ...
  const handleNext = () => {
    if (activeStep === 0) {
      const fields = ["profile", "bio", "question1", "question2", "question3", "fb", "twitter", "insta", "lkd", "ytb"];
      ...
      const fields = ["coordinator?.name", "coordinator?.email", "coordinator?.phone", "coordinator?.designation", "question8", "question9"];
      const errorCheck = FieldsErrorChecker(fields);
      if (errorCheck) {
        fields.forEach((field) => {
          formik?.setFieldTouched(field, true);
...
export default function OnboardModalParent({ details }) {
  ...
  return (
    <Modal
      ...
      <Box sx={OnboardModalstyle}>
        <OnboardStepper handleClose={handleClose} details={details} />
      </Box>
    </Modal>
  );
}
